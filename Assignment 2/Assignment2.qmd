...
dfData2=dfData2 %>% group_by(ethblack) %>% mutate(group_m_school = mean(school))
# Get average group mean of age per individual by ethnicity
dfData2=dfData2 %>% group_by(ethblack) %>% mutate(group_m_age = mean(age))
# Get average group mean of age squared per individual by ethnicity
dfData2=dfData2 %>% group_by(ethblack) %>% mutate(group_m_agesq = mean(agesq))
# Get average group mean of urban per individual by ethnicity
dfData2=dfData2 %>% group_by(ethblack) %>% mutate(group_m_urban = mean(urban))
# Get average group mean of regne per individual by ethnicity
dfData2=dfData2 %>% group_by(ethblack) %>% mutate(group_m_regne = mean(regne))
# Get average group mean of regnc per individual by ethnicity
dfData2=dfData2 %>% group_by(ethblack) %>% mutate(group_m_regnc = mean(regnc))
# Get average group mean of regw per individual by ethnicity
dfData2=dfData2 %>% group_by(ethblack) %>% mutate(group_m_regw = mean(regw))
# Get average group mean of asvabc per individual by ethnicity
dfData2=dfData2 %>% group_by(ethblack) %>% mutate(group_m_asvabc = mean(asvabc))

# Mundlak estimation
Mundlak_reg <-  plm(earnings ~ group_m_school + school + ethblack +age+agesq + urban + regne + regnc + regw + asvabc , data = dfData2, index = c("id","time"), model="pooling")
summary(Mundlak_reg)

#Mundlak_gls <- gls(earnings ~ group_m_school + school + ethblack +age+agesq + urban + regne + regnc + regw + asvabc, data = dfData2)
